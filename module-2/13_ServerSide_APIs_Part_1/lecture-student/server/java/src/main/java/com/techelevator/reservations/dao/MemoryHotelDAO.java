...
public class MemoryHotelDAO implements HotelDAO {
    ...
    public List<Hotel> filterByState(String state) {
        ...
        //loop through existing hotels and add to the filteredList if the state matches
        for(Hotel hotel : hotels){
            if (hotel.getAddress().getState().toUpperCase().equals(state.toLowerCase())){
    ...
    public List<Hotel> filterByCity(String city) {
        ...
        //loop through existing hotels and add to the filteredList if the state matches
        for(Hotel hotel : hotels){
            if (hotel.getAddress().getCity().toUpperCase().equals(city.toLowerCase())){
                filteredList.add(hotel);
            }
        }
        return filteredList;

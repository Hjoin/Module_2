...
Afterwards, run the unit tests in `src/test/java/com/techelevator/auctions/model/AuctionValidationTest.java` to verify that you have the correct validations in place. The messages provided must match EXACTLY in order for tests to pass, including the backticks (` => backtick, ' => not a backtick). The backtick key is in the upper left hand corner of your keyboard.

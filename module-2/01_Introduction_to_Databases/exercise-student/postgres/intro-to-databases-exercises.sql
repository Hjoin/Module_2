...
FROM city
WHERE countrycode = 'USA' AND population > 1000000 AND district = 'Texas'; 
...
SELECT name, continent, gnp, population, gnp/population AS per_capita_gnp
FROM country
WHERE continent = 'Europe';

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT name, indepyear, 2021 - indepyear AS years_since_independence
FROM country
